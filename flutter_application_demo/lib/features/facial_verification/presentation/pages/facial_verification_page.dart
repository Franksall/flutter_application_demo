...
class FacialVerificationPage extends ConsumerStatefulWidget {
  const FacialVerificationPage({super.key});

  @override
  ConsumerState<FacialVerificationPage> createState() =>
      _FacialVerificationPageState();
}

class _FacialVerificationPageState
    extends ConsumerState<FacialVerificationPage> {
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        if (_isFaceCaptured) {
                          context.go(AppConstants.biometricValidationRoute);
                        } else {
                          try {
                            // LLAMADA AL BYPASS DE PYTHON
                            // generará el log  terminal
                            await getIt<ApiClient>().get(
                              '/api/session-token/12345678',
                            );
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Bypass exitoso: Rostro validado en Backend',
                                ),
                              ),
                            );
                          } catch (e) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                                  'Error de conexión con el Backend: $e',
                                ),
                              ),
                            );
                          }
      ...
      child: Column(
        ...
        children: [
          ...
          _buildTipItem(
            ...
            icon: Icons.check,
            text:
                'Retira gafas, gorras o cualquier accesorio que cubra tu rostro',
